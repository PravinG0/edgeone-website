/**
 * CoreSolutions.tsx
 * Bento-grid "Our Core Solutions" section with:
 *  - GSAP scroll-triggered staggered card reveal
 *  - 3-D tilt on mouse-move per card
 *  - Mouse-spotlight radial gradient
 *  - Animated conic-gradient border on hover
 *  - Floating blob glow
 * Ported & adapted from PravinG0/edgeone---Our-Core-Solutions-and-Industries-We-Empower-section
 */

import { useEffect, useRef } from "react";
import { gsap } from "gsap";
import { ScrollTrigger } from "gsap/ScrollTrigger";
import {
  Brain, Network, Cpu, HeartPulse, HardDrive, Workflow, ArrowUpRight,
  type LucideIcon,
} from "lucide-react";

gsap.registerPlugin(ScrollTrigger);

type Solution = {
  title: string;
  desc: string;
  icon: LucideIcon;
  accent: string;
  span: string;
  tag: string;
};

const SOLUTIONS: Solution[] = [
  {
    title: "Edge AI Solutions",
    desc: "Deploy AI directly at the edge with ultra-low latency processing, real-time analytics, and intelligent automation — no cloud dependency, full control at the point of action.",
    icon: Brain,
    accent: "oklch(0.72 0.22 255)",
    span: "lg:col-span-7",
    tag: "01 / Intelligence",
  },
  {
    title: "Industrial IoT Systems",
    desc: "Smart sensors, telemetry engines, monitoring gateways, and enterprise IoT platforms — a complete IoT stack from device to dashboard.",
    icon: Network,
    accent: "oklch(0.78 0.18 195)",
    span: "lg:col-span-5",
    tag: "02 / Connectivity",
  },
  {
    title: "GPU Servers & AI Infrastructure",
    desc: "Custom-built AI workstations, GPU clusters, inference servers, and enterprise hardware powering ML pipelines, LLM inference, and computer vision systems.",
    icon: Cpu,
    accent: "oklch(0.65 0.25 285)",
    span: "lg:col-span-5",
    tag: "03 / Compute",
  },
  {
    title: "Smart Healthcare Technologies",
    desc: "Remote patient monitoring, medical device integrations, and hospital intelligence systems that bring real-time IoT and edge AI together for clinical impact.",
    icon: HeartPulse,
    accent: "oklch(0.68 0.24 15)",
    span: "lg:col-span-4",
    tag: "04 / Healthcare",
  },
  {
    title: "Edge Computing Devices",
    desc: "Industrial PCs, rugged systems, panel PCs, and custom edge hardware — engineered for the harshest environments and mission-critical uptime.",
    icon: HardDrive,
    accent: "oklch(0.70 0.20 255)",
    span: "lg:col-span-4",
    tag: "05 / Hardware",
  },
  {
    title: "Enterprise Automation",
    desc: "AI-powered workflows, smart dashboards, and operational intelligence platforms with private cloud, storage, and IaaS for a complete enterprise stack.",
    icon: Workflow,
    accent: "oklch(0.65 0.25 285)",
    span: "lg:col-span-4",
    tag: "06 / Automation",
  },
];

export function CoreSolutions() {
  const sectionRef = useRef<HTMLElement | null>(null);
  const headlineRef = useRef<HTMLHeadingElement | null>(null);
  const cardsRef   = useRef<HTMLDivElement | null>(null);

  useEffect(() => {
    const ctx = gsap.context(() => {
      // Headline stagger reveal
      gsap.from(".cs-eyebrow, .cs-headline > span, .cs-sub", {
        scrollTrigger: { trigger: headlineRef.current, start: "top 80%" },
        y: 40, opacity: 0, duration: 1, ease: "power3.out", stagger: 0.08,
      });

      // Cards staggered 3-D rise
      const cards = gsap.utils.toArray<HTMLElement>(".cs-card");
      if (!cards.length) return;

      gsap.from(cards, {
        scrollTrigger: { trigger: cardsRef.current, start: "top 75%" },
        y: 80, opacity: 0, scale: 0.94, rotateX: -8,
        duration: 0.9, ease: "power3.out", stagger: 0.1,
      });

      // Subtle parallax per card
      cards.forEach((card, i) => {
        gsap.to(card, {
          y: i % 2 === 0 ? -28 : 28,
          ease: "none",
          scrollTrigger: { trigger: card, start: "top bottom", end: "bottom top", scrub: true },
        });
      });

      // 3-D mouse-tilt + spotlight per card
      cards.forEach((card) => {
        const onMove = (e: MouseEvent) => {
          const r = card.getBoundingClientRect();
          const x = ((e.clientX - r.left) / r.width)  * 100;
          const y = ((e.clientY - r.top)  / r.height) * 100;
          card.style.setProperty("--mx", `${x}%`);
          card.style.setProperty("--my", `${y}%`);
          const rx = ((e.clientY - r.top)  / r.height - 0.5) * -8;
          const ry = ((e.clientX - r.left) / r.width  - 0.5) *  8;
          gsap.to(card, { rotateX: rx, rotateY: ry, duration: 0.4, ease: "power2.out", transformPerspective: 900 });
        };
        const onLeave = () => {
          gsap.to(card, { rotateX: 0, rotateY: 0, duration: 0.6, ease: "power3.out" });
        };
        card.addEventListener("mousemove", onMove);
        card.addEventListener("mouseleave", onLeave);
      });
    }, sectionRef);

    return () => ctx.revert();
  }, []);

  return (
    <section
      ref={sectionRef}
      className="relative isolate overflow-hidden py-32 px-6 bg-background"
      aria-labelledby="core-solutions"
    >
      {/* Background layers */}
      <div
        className="pointer-events-none absolute inset-0 opacity-40"
        style={{
          backgroundImage:
            "linear-gradient(to right, hsl(221.2 83.2% 53.3% / 0.08) 1px, transparent 1px), linear-gradient(to bottom, hsl(221.2 83.2% 53.3% / 0.08) 1px, transparent 1px)",
          backgroundSize: "60px 60px",
        }}
      />
      <div
        className="pointer-events-none absolute -top-40 left-1/2 h-[700px] w-[700px] -translate-x-1/2 rounded-full blur-3xl"
        style={{ background: "radial-gradient(circle, hsl(221.2 83.2% 53.3% / 0.2), transparent 60%)" }}
      />
      <div className="pointer-events-none absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-primary/60 to-transparent" />

      {/* Header */}
      <header ref={headlineRef} className="relative mx-auto max-w-4xl text-center">
        <p className="cs-eyebrow mb-5 inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.35em] text-primary">
          <span className="h-px w-8 bg-primary/60" />
          Our Core Solutions
          <span className="h-px w-8 bg-primary/60" />
        </p>
        <h2 id="core-solutions" className="cs-headline text-5xl font-black leading-[1.05] tracking-tight md:text-6xl font-outfit">
          <span className="block">End-to-End Intelligence.</span>
          <span className="block text-gradient">One Platform.</span>
        </h2>
        <p className="cs-sub mx-auto mt-6 max-w-2xl text-base text-zinc-400 md:text-lg font-medium">
          From IoT at the edge to enterprise-grade cloud infrastructure, every solution is built for
          performance, scale, and real-world reliability.
        </p>
      </header>

      {/* Bento grid */}
      <div
        ref={cardsRef}
        className="relative mx-auto mt-20 grid max-w-7xl gap-5 lg:grid-cols-12 lg:grid-rows-[auto_auto_auto]"
        style={{ perspective: "1200px" }}
      >
        {SOLUTIONS.map((s) => {
          const Icon = s.icon;
          return (
            <article
              key={s.title}
              className={`cs-card group relative overflow-hidden rounded-2xl p-7 transition-shadow duration-500 will-change-transform ${s.span}`}
              style={{
                ["--accent" as string]: s.accent,
                background: "linear-gradient(145deg, hsl(222.2 84% 6% / 0.85), hsl(222.2 84% 4.9% / 0.70))",
                backdropFilter: "blur(18px)",
                border: "1px solid hsl(221.2 83.2% 53.3% / 0.18)",
              }}
            >
              {/* Mouse spotlight */}
              <div
                aria-hidden
                className="pointer-events-none absolute inset-0 opacity-0 transition-opacity duration-300 group-hover:opacity-100"
                style={{
                  background:
                    "radial-gradient(450px circle at var(--mx,50%) var(--my,50%), color-mix(in oklab, var(--accent) 25%, transparent), transparent 60%)",
                }}
              />

              {/* Animated conic border on hover */}
              <div
                aria-hidden
                className="pointer-events-none absolute inset-0 rounded-2xl opacity-0 transition-opacity duration-500 group-hover:opacity-100"
                style={{
                  padding: "1px",
                  background:
                    "conic-gradient(from 0deg, transparent, color-mix(in oklab, var(--accent) 80%, transparent), transparent 40%)",
                  WebkitMask: "linear-gradient(#000 0 0) content-box, linear-gradient(#000 0 0)",
                  WebkitMaskComposite: "xor",
                  maskComposite: "exclude",
                  animation: "orbit-spin 6s linear infinite",
                }}
              />

              {/* Tag */}
              <div className="relative mb-6 flex items-center justify-between text-[10px] font-semibold uppercase tracking-[0.3em] text-zinc-500">
                <span>{s.tag}</span>
                <span
                  className="h-1.5 w-1.5 rounded-full"
                  style={{
                    background: s.accent,
                    boxShadow: `0 0 12px ${s.accent}`,
                  }}
                />
              </div>

              {/* Icon */}
              <div className="relative mb-6 inline-flex">
                <div
                  className="absolute inset-0 rounded-xl blur-xl"
                  style={{ background: `color-mix(in oklab, ${s.accent} 50%, transparent)` }}
                />
                <div
                  className="relative grid h-14 w-14 place-items-center rounded-xl border"
                  style={{
                    background: `color-mix(in oklab, ${s.accent} 18%, transparent)`,
                    borderColor: `color-mix(in oklab, ${s.accent} 45%, transparent)`,
                  }}
                >
                  <Icon className="h-7 w-7" style={{ color: s.accent }} />
                </div>
              </div>

              <h3 className="relative text-xl font-bold tracking-tight text-foreground md:text-2xl font-outfit">
                {s.title}
              </h3>
              <p className="relative mt-3 text-sm leading-relaxed text-zinc-400 md:text-[15px]">
                {s.desc}
              </p>

              <a
                href="#"
                className="relative mt-6 inline-flex items-center gap-2 text-sm font-semibold transition-opacity hover:opacity-70"
                style={{ color: s.accent }}
              >
                Learn More
                <ArrowUpRight className="h-4 w-4 transition-transform duration-300 group-hover:translate-x-1 group-hover:-translate-y-1" />
              </a>

              {/* Floating blob */}
              <div
                aria-hidden
                className="pointer-events-none absolute -bottom-16 -right-16 h-48 w-48 rounded-full opacity-25 blur-3xl"
                style={{
                  background: s.accent,
                  animation: "blob-float 14s ease-in-out infinite",
                }}
              />
            </article>
          );
        })}
      </div>
    </section>
  );
}
